...
const createEventInterest = async (eventInterest) => {
  ...
  try {
    const query = "INSERT INTO event_interests (event_id, category) VALUES ($1, $2) RETURNING *";
...
const updateEventInterests = async (event_id, interests) => {
    try {
      ...
      const insertQuery = "INSERT INTO event_interests (event_id, category) VALUES ($1, $2)";
      const insertValues = interests.map((interest) => [event_id, interest]);
      await db.tx(async (transaction) => {
        const queries = insertValues.map((values) => transaction.none(insertQuery, values));
...
const deleteEventInterest = async (eventInterest) => {
  const { event_id, category } = eventInterest;
  try {
    const query = "DELETE FROM event_interests WHERE event_id = $1 AND category = $2 RETURNING *";
